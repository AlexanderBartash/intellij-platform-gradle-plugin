...
class IntelliJPlatformRepositoriesHelper(
     ...
     */
    internal fun localPlatformArtifactsRepository(repositoryName: String, action: IvyRepositoryAction = {}): IvyArtifactRepository {
     ...
     * @see org.jetbrains.intellij.platform.gradle.models.IvyModule.toAbsolutePathLocalPluginIvyArtifacts
     */
    internal fun localPlatformBundledArtifacts(repositoryName: String, action: IvyRepositoryAction = {}): IvyArtifactRepository {
        return createExclusiveIvyRepository(
            repositoryName,
            setOf(
    ...
    private fun createIvyArtifactRepository(
        ...
        if (null == ivyLocationPath) {
            ivyPattern("/to-be-updated/[organization]-[module]-[revision].[ext]")
            artifactPattern("/to-be-updated/[artifact]")
            // TODO
            return@ivy
        }
        ...
        // We can not rid of this leading separator as long as the repository created by localPlatformArtifactsRepository
    ...
    companion object {
         ...
         * @see org.jetbrains.intellij.platform.gradle.extensions.IntelliJPlatformDependenciesHelper.registerIntellijPlatformIvyRepo
         */
        internal fun updateDynamicBundledIvyArtifactsRepository(
            repositories: RepositoryHandler,
            settingsRepositories: RepositoryHandler,
            ivyLocationPath: Path,
            artifactLocationPath: Path
        ) {
            val repositoryName = Repositories.LOCAL_INTELLI_J_PLATFORM_BUNDLED_ARTIFACTS

            // It may be called more than once
            var repository = repositories.findByName(repositoryName)
            if (null == repository) {
                repository = settingsRepositories.findByName(repositoryName)
                if (null == repository) {
                    // TODO
                    return
                }
            ...
            if (repository !is IvyArtifactRepository) {
                // TODO
                return
            }
            ...
            repository.ivyPattern("$ivyPath/[organization]-[module]-[revision].[ext]")
            ...
            repository.artifactPattern("$optionalPath/([type]/)[artifact](.[ext])")
        }
    }
}
